...
class CallbackModule(DefaultCallbackModule):
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        self._display.display(
            self.banner(f"{self._time()} 📦 Play: {stringc(name, 'cyan')}")
        )
    ...
    def v2_runner_on_ok(self, result):
        ...
        if action == "debug":
            msg = result_data.get("msg") or result_data.get("var", "")
            self._display.display(f"🟢 DEBUG: {pformat(msg)}")

        elif action == "pause":
            prompt = result_data.get("user_input") or result_data.get("msg", "Paused")
            self._display.display(f"⏸️ PAUSE: {prompt}")

        else:
            self._display.display(f"🟢 OK: {name}")
    ...
    def v2_runner_on_skipped(self, result):
        task_name = result._task.get_name()
        self._display.display(
            f"{self._time()} ⚪ SKIPPED: {stringc(task_name, 'magenta')}"
        )
